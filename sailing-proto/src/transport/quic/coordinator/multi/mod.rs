  ...
  ID: IdentitySource<I>,
{
    ...
    S: StableStore<NodeId = I>,
  {
    ...
    self.purge_unknown_signal(&key);
    // Same cap-raise as `create_group`, for the same reason and at the same point: a founded group
    // widens the tracked-peer union exactly as an ordinary one does, and leaving the raise to the
    // next pump lets an accept arriving in the gap be statelessly refused. A direct coordinator
    // caller — one that admits and then waits for the group's peers to dial in — has no next pump
    // to rescue it, so the new group simply strands.
    self.bridge.raise_max_connections(self.effective_cap());
    Ok(())
  }
      ...
      loop {
        ...
        if self.bridge.is_authenticating(h) {
          ...
          for (flags, group_bytes, generation, message) in entries {
            ...
            //
            // DELIBERATELY DEBT-ONLY, exactly as the stream demux: a source named by a
            // latched-CLOSED park is refused at the admission doors but must keep receiving here,
            // because the cure chain that resolves that park rides the consumed source's frames.
