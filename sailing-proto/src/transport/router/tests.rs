...
fn binds_peer_on_validation_and_routes() {
  ...
  assert!(router.route(&[], 0, 7, &hb(10)));
  pump(&mut router, id, &mut peer);
  let mut got = Vec::new();
  peer.poll_decoded(&mut got).unwrap();
  let got: std::vec::Vec<_> = got.into_iter().map(|(_, _, _, m)| m).collect();
...
fn decodes_inbound_messages_with_their_peer() {
  ...
  assert_eq!(
    delivered,
    std::vec![(bytes::Bytes::new(), 0u8, 0u64, 7, hb(7))]
  );
}
...
fn route_to_unknown_peer_is_dropped() {
  let mut router = R::new();
  assert!(!router.route(&[], 0, 99, &hb(1)), "no conn to peer 99");
}
