...
use sailing_driver::{
  ...
  jittered,
  shared::{
    CompletionOutcome, EngineGate, InflightBudget, ParkedFailover, ParkedQuery, Pending,
    PendingAck, Routing,
  },
...
/// [`MultiReactorStreamDriver::rearm_cue_retry`]). Fixed, and deliberately coarse relative to the
...
/// [`agnostic::Runtime`]; `G` the group id; `Rec` the record layer the factories build; `E` the
/// storage engine, which defaults to the in-memory [`GroupEngine`] every constructor builds.
///
...
/// Storage is an OWNED engine behind the [`MultiEngine`] seam, and each crank runs one
/// engine-wide barrier over every hosted group.
///
...
///
/// The DEFAULT constructors build a [`GroupEngine`]: the shared IN-MEMORY reference engine (its own
/// doc: floors survive exactly what the engine survives, and a durable mirror is the planned
/// persistent form). A process crash therefore loses ALL consensus state under the default — that
/// shape is for tests, single-process deployments, and as the reference a persistent engine is
/// validated against. [`bind_with_engine`](Self::bind_with_engine) takes the caller's own
/// [`MultiEngine`] instead, which is the whole point of the seam: durability is that engine's to
/// provide, and this host is agnostic to it. What is NOT here yet is the recovery half —
/// `restore_group` reconnects a group within the SAME live process (after a driver-level
/// teardown), and a recover-every-group-from-a-durable-engine entry point arrives with M-1b. The
/// lifecycle and event tails are best-effort TELEMETRY for observability, never a correctness
/// feed: a dropped tail entry loses a notification, never consensus state.
pub struct MultiReactorStreamDriver<R, G, I, F, Rec, E = GroupEngine<G, I>>
where
  R: Runtime,
...
/// The constructors, which name the engine CONCRETELY: a defaulted type parameter does not infer
/// in expression position, so binding these to [`GroupEngine`] is what keeps every call site
/// turbofish-free. Everything a bound host does lives on the generic block below.
impl<R, G, I, F, Rec> MultiReactorStreamDriver<R, G, I, F, Rec, GroupEngine<G, I>>
where
  R: Runtime,
  G: GroupId + Send,
...
impl<R, G, I, F, Rec, E> MultiReactorStreamDriver<R, G, I, F, Rec, E>
where
  R: Runtime,
  G: GroupId + Send,
  I: sailing_proto::NodeId + Send,
  ...
  E: MultiEngine<G, I>,
{
  ...
  pub async fn bind_with_engine(
    ...
    let coord = MultiStreamCoordinator::new();
    Ok(Self::from_parts(
      coord, listener, peers, dialer, acceptor, driver_cfg, engine,
    ))
  }
  ...
  fn restore_group(
    ...
    match result {
      Ok(()) => {
        // Re-sync to the LIVE restored counter, and to NOTHING ELSE: restart replay re-applies
        // lineage moves (a merge freeze above all) whose event-time mirror may have died with the
        // crash — replay surfaces no events, so this is the fold that heals the engine record
        // (INV-LINEAGE across the crash window). The admitted `generation` is deliberately absent:
        // the record is the id's fork/lineage EVIDENCE, read back as a global proof by the thaw
        // witness and as the fork replay guard's seed, and a catalog claim is neither — recording
        // one would let this host witness a discharge no replica ever performed, and seed a guard
        // that folds the group's next fork away.
  ...
  /// therefore NOT be deleted on the strength of a green suite. Its BODY is pinned directly, by
  /// `a_harvest_of_one_latched_routing_fail_stops_every_hosted_group`.
